...
// Lấy thông tin đơn hàng (chỉ cho phép review đơn hàng đã hoàn thành)
...
$details_stmt = $conn->prepare("SELECT od.product_id, od.quantity, p.name, p.image 
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['reviews'])) {
    ...
    $review_stmt = $conn->prepare("INSERT INTO reviews (user_id, product_id, rating, comment, created_at) VALUES (?, ?, ?, ?, NOW())");
    ...
    // Sau khi đánh giá xong thì quay về chi tiết đơn hàng
...
<head>
    ...
    <title>Đánh giá sản phẩm - Đơn hàng #<?php echo htmlspecialchars($order_id); ?></title>
    <link rel="stylesheet" href="assets/css/style.css">
    <link rel="stylesheet" href="assets/css/form.css">
    <style>
        .review-form { margin-bottom: 20px; padding: 15px; border: 1px solid #eee; border-radius: 8px; }
        .review-form img { max-width: 80px; margin-right: 15px; border-radius: 6px; }
        .review-item { display: flex; align-items: flex-start; margin-bottom: 15px; }
        .review-item textarea { width: 100%; min-height: 80px; }
    </style>
</head>
<body>
